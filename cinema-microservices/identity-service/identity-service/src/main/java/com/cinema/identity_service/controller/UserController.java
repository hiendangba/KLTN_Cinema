...
public class UserController extends BaseController {
    ...
    public ResponseEntity<APIResponse<RegisterCustomerResponse>> register(@Valid @RequestBody RegisterCustomerRequest registerCustomerRequest, HttpServletResponse response) {
        RegisterCustomerResponse registerCustomerResponse = userService.registerCustomer(registerCustomerRequest,response);
        return created(registerCustomerResponse);
    }

    @PostMapping("/verify-otp")
    public ResponseEntity<APIResponse<VerifyResponse>> verifyOTP(@Valid @RequestBody VerifyRequest verifyRequest, HttpServletRequest request) {
        VerifyResponse verifyResponse = userService.verifyOTP(verifyRequest,request);
        return ok(verifyResponse);
    }

    @PostMapping("/forgot-password")
    public ResponseEntity<APIResponse<ForgotPasswordResponse>> forgotPassword(@Valid @RequestBody ForgotPasswordRequest forgotPasswordRequest, HttpServletResponse response){
