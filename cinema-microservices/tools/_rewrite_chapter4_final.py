...
def find_paragraph(doc, startswith_text):
    for p in doc.paragraphs:
        if p.text.strip().startswith(startswith_text):
            return p
    raise ValueError(f"Paragraph not found: {startswith_text}")


def replace_paragraph_text(doc, startswith_text, new_text):
    p = find_paragraph(doc, startswith_text)
    p.text = new_text
    return p
...
def main():
    doc_path = Path(os.environ["DOCX_PATH"])
    doc = Document(str(doc_path))

    # 4.1 rewrite
    replace_paragraph_text(
        doc,
        "Hệ thống CinemaStar được triển khai theo mô hình phân tách rõ giữa frontend",
    ...
    replace_paragraph_text(
        doc,
        "Mọi yêu cầu từ giao diện đều đi qua gateway trước khi được chuyển đến service tương ứng",
    ...
    replace_paragraph_text(
        doc,
        "Việc chia backend thành các service độc lập",
    ...
    replace_paragraph_text(
        doc,
        "Tầng dữ liệu và hạ tầng bao gồm cơ sở dữ liệu",
    ...
    replace_paragraph_text(
        doc,
        "Nhờ phân lớp như vậy, hệ thống vừa đảm bảo tính nhất quán dữ liệu",
    ...
    replace_paragraph_text(
        doc,
        "Tổng thể, mô hình triển khai của CinemaStar đáp ứng được yêu cầu vận hành ổn định",
    ...
    replace_paragraph_text(
        doc,
        "Quy trình triển khai được tổ chức theo từng bước rõ ràng",
    ...
    replace_paragraph_text(
        doc,
        "Trước hết, mã nguồn của hệ thống được chuẩn bị theo hai khối chính",
    ...
    replace_paragraph_text(
        doc,
        "Sau giai đoạn đóng gói, hệ thống được cấu hình để các lớp có thể giao tiếp",
    ...
    replace_paragraph_text(
        doc,
        "Khi đưa lên môi trường triển khai, hệ thống được kiểm tra theo từng lớp",
    ...
    replace_paragraph_text(
        doc,
        "Nhìn chung, quy trình triển khai của CinemaStar được thiết kế theo hướng có kiểm soát",
    ...
    replace_paragraph_text(
        doc,
        "Mục tiêu của quá trình kiểm thử là xác minh tính đúng đắn của các rule nghiệp vụ",
    ...
    replace_paragraph_text(
        doc,
        "Trong quá trình phát triển, kiểm thử được triển khai theo hai lớp",
    ...
    replace_paragraph_text(
        doc,
        "Ở mức đơn vị, các test chạy trực tiếp trong mã nguồn bằng bộ JUnit/Mockito",
    ...
    # Unit test section body
    unit_heading = find_paragraph(doc, "4.2.3. Kiểm thử đơn vị")
    unit_style = find_paragraph(doc, "4.2.2. Công cụ và môi trường kiểm thử").style
    unit_body_style = find_paragraph(doc, "Mục tiêu của quá trình kiểm thử là xác minh tính đúng đắn của các rule nghiệp vụ").style
    unit_p1 = insert_paragraph_after(
        unit_heading,
        ...
        style=unit_body_style,
    )
    unit_p2 = insert_paragraph_after(
        unit_p1,
    ...
    # API test section body and result paragraphs are already present, but style them/refresh wording.
    api_heading = find_paragraph(doc, "4.2.4. Kiểm thử chức năng API bằng Postman/Newman")
    ...
    result_heading = find_paragraph(doc, "4.2.5. Kết quả kiểm thử")
    result_body1 = find_paragraph(doc, "Sau khi chạy collection bằng Newman")._p
    result_body2 = find_paragraph(doc, "Ngoài kết quả tổng quan, báo cáo Newman")._p

    # Refresh wording of the result paragraphs if desired
    find_paragraph(doc, "Sau khi chạy collection bằng Newman").text = "Sau khi chạy collection bằng Newman trên môi trường production, hệ thống thực thi 19 request với tổng cộng 57 assertion. Kết quả cho thấy toàn bộ assertion đều thành công và không có test thất bại."
    find_paragraph(doc, "Ngoài kết quả tổng quan, báo cáo Newman").text = "Ngoài kết quả tổng quan, báo cáo Newman còn cung cấp thông tin chi tiết cho từng request. Ví dụ, request POST /api/films/search được sử dụng để kiểm tra chức năng tìm kiếm phim. Kết quả cho thấy API trả về mã trạng thái 200 OK, tỷ lệ kiểm thử thành công đạt 100% và thời gian phản hồi trung bình là 202 ms."

    # Ensure styles on inserted sections are consistent
    api_body.style = unit_body_style
    result_heading.style = unit_style
