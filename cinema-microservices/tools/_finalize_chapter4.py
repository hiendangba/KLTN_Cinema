...
def find_para(doc, startswith_text):
...
def replace_text(doc, startswith_text, new_text):
    p = find_para(doc, startswith_text)
    p.text = new_text
    return p
...
def main():
    ...
    body_style = find_para(doc, "Mục tiêu của quá trình kiểm thử").style
    heading_style = find_para(doc, "4.2.2. Công cụ và môi trường kiểm thử").style

    # Insert unit test details under 4.2.3
    unit_heading = find_para(doc, "4.2.3. Kiểm thử đơn vị")
    unit_p1 = insert_after(
        unit_heading,
    ...
    # Insert API test body under 4.2.4
    api_heading = find_para(doc, "4.2.4. Kiểm thử chức năng API bằng Postman/Newman")
    insert_after(
        api_heading,
    ...
    # Optional polish for result paragraphs
    replace_text(
        doc,
        "Sau khi chạy collection bằng Newman trên môi trường production",
    ...
    replace_text(
        doc,
        "Ngoài kết quả tổng quan, báo cáo Newman còn cung cấp thông tin chi tiết",
